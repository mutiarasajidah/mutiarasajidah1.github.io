...
$sql = "SELECT p.name, SUM(t.jumlah) as total_jumlah, t.tanggal 
        FROM transaksi t 
        JOIN products p ON t.product_id = p.id 
        WHERE DATE(t.tanggal) = ?
        GROUP BY p.name, t.tanggal";
...
<main>
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            echo "<tr><td>" . htmlspecialchars($row["name"]). "</td><td>" . htmlspecialchars($row["total_jumlah"]). "</td><td>" . htmlspecialchars($row["tanggal"]). "</td></tr>";
        }
        echo "</table>";
    } else {
